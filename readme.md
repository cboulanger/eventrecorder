...
by that text when the script is evaluated. 

**Macros** are enclosed in "define" and "end" and are also evaluated at runtime.
They additionally will replace "$1", "$2" etc with the value of the the nth
argument passed to the name of the macro in the script.

Do not expect any functionality beyond these rather primitive features, everything else should be implemented in the 
